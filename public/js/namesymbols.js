...
const instructionText = `
		...
		<p>You can click on the tile to reveal its name if you get stuck.</p>`;
...
class NameSymbols {
	...
	async init(){
		...
		this.restartButton.addEventListener('click', () => {
			...
			SceneManager.goToScene('namesymbols');
		});
	...
	randomizeTiles(){
		...
		this.currentTile.style.display = 'none';
	...
	moveCurrentTile(){
		...
		moveTileToCorrect(this.currentTile, targetOutline);
		console.log('Moving');
	}
	...
	gameWon(){
		...
		if (!isAchievementComplete('symbolcomplete')){
			storeAchievementInStorage('symbolcomplete');
		}
		if (!isAchievementComplete('symbolprodigy')){
			if (!this.mistakes){
				storeAchievementInStorage('symbolprodigy');
			}
		}
		if (!isAchievementComplete('symbolauth')){
			if(timeInMilliSeconds < 240000) {
				storeAchievementInStorage('symbolauth');
			}	
		}
		if (!isAchievementComplete('symbolchemist')){
			if(timeInMilliSeconds < 360000){
				storeAchievementInStorage('symbolchemist');
			}			
		}
	}
}
